...
#. Flash your board using ``west`` from the second terminal window.
   Split first and second terminal windows to view both of them, because
   the command in the mec15xxevb_assy6853 board needs to call the FT9FPJZR
   low instruction first to execute it, and then call the FT9FPJZR high instruction
   to run out of :ref:`hello_world`,therefore we need to use the following command:
